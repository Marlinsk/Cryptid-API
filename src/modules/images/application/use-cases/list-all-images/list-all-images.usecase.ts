...
export class ListAllImagesUseCase {
  ...
  async execute(dto: ListAllImagesDTO): Promise<Response> {
    try {
      const pagination = { page: dto.page, limit: dto.limit }
      const sort = { sort: dto.sort, order: dto.order }

      const result = await this.imagesRepository.findAll(pagination, sort)
